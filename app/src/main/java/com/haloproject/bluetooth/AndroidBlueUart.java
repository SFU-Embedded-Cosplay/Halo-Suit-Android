...
/**
 * Bluetooth LE UART library for Android, provided by Adafruit.
...
public class AndroidBlueUart extends BluetoothGattCallback implements BluetoothAdapter.LeScanCallback {
    ...
    public static String BLE_UART_ADAPTER_ADDRESS = "D4:23:09:DB:3A:16";
    ...
    // Interface for a AndroidBlueUart client to be notified of UART actions.
    ...
    public AndroidBlueUart(Context context) {
        super();
...
//        TODO: figure out why this code was creating a null pointer exception
...
//        gatt.readCharacteristic(disManuf);
    ...
    public void onLeScan(BluetoothDevice device, int rssi, byte[] scanRecord) {
        ...
        if (device.getAddress().equals(BLE_UART_ADAPTER_ADDRESS)) {
    ...
    private void notifyOnConnected(AndroidBlueUart uart) {
    ...
    private void notifyOnConnectFailed(AndroidBlueUart uart) {
        for (Callback cb : callbacks.keySet()) {
            if (cb != null) {
    ...
    private void notifyOnDisconnected(AndroidBlueUart uart) {
        for (Callback cb : callbacks.keySet()) {
            if (cb != null) {
    ...
    private void notifyOnReceive(AndroidBlueUart uart, BluetoothGattCharacteristic rx) {
        for (Callback cb : callbacks.keySet()) {
            if (cb != null ) {
